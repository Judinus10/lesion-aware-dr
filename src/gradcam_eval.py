...
class EyePacsDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if self.transform is not None:
            x = self.transform(image=rgb_uint8)["image"]  # torch tensor CxHxW float
        else:
            x = torch.from_numpy(rgb_uint8).permute(2, 0, 1).float() / 255.0

        return x, label, fname, rgb_uint8
...
def find_last_conv_layer(model: torch.nn.Module) -> torch.nn.Module:
    ...
    if last_conv is None:
        raise RuntimeError("No Conv2d layer found. Can't run CAM.")
    return last_conv
...
# ✅ KEY FIX: custom collate so rgbs + names stay as lists (no numpy/object weirdness)
def collate_keep_meta(batch):
    xs, ys, names, rgbs = zip(*batch)
    x = torch.stack(xs, dim=0)
    y = torch.tensor(ys, dtype=torch.long)
    return x, y, list(names), list(rgbs)
...
def main():
    ...
    dl = DataLoader(
        ...
        collate_fn=collate_keep_meta,   # ✅ FIX
    )
    ...
    for xb, yb, names, rgbs in dl:
        ...
        # IMPORTANT: CAM needs gradients internally (don’t wrap this in no_grad)
        grayscale_cams = cam(input_tensor=xb, targets=targets)  # (B, H, W) [0..1]
        ...
        for i in range(len(names)):
            ...
            rgb = rgbs[i]  # already uint8 np array HxWx3
            cam_map = grayscale_cams[i]

            # save raw cam for IoU later
            np.save(npy_dir / f"{stem}_cam.npy", cam_map.astype(np.float32))

            overlay = overlay_cam_on_image(rgb, cam_map, alpha=args.alpha)
            out_name = f"{exported:04d}__true{y_true[i]}__pred{pred[i]}__{stem}.png"
            cv2.imwrite(str(png_dir / out_name), cv2.cvtColor(overlay, cv2.COLOR_RGB2BGR))

            exported += 1

    print(f"[GradCAM] Done. Exported {exported} CAMs to: {out_dir}")
